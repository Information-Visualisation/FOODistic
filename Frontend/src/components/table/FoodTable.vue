        ...
        <nav>
            <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
                <button class="nav-link active" id="nav-food-tab" data-bs-toggle="tab" data-bs-target="#nav-food"
                    type="button" role="tab" aria-controls="nav-food" aria-selected="true"
                    @click="setTabIndex(0)">Food</button>
                <button class="nav-link" id="nav-nutrition-tab" data-bs-toggle="tab" data-bs-target="#nav-nutrition"
                    type="button" role="tab" aria-controls="nav-nutrition" aria-selected="false"
                    @click="setTabIndex(1)">Nutrition</button>
                <button class="nav-link" id="nav-allergies-tab" data-bs-toggle="tab" data-bs-target="#nav-allergies"
                    type="button" role="tab" aria-controls="nav-allergies" aria-selected="false"
                    @click="setTabIndex(2)">Allergies</button>
                <button class="nav-link" id="nav-recipes-tab" data-bs-toggle="tab" data-bs-target="#nav-recipes"
                    type="button" role="tab" aria-controls="nav-recipes" aria-selected="false"
                    @click="setTabIndex(3)">Recipes</button>
            </div>
        </nav>
        <!-- content -->
        <div class="tab-content" id="nav-tabContent">
            <div class="tab-pane fade show active no-rows" id="nav-food" role="tabpanel" aria-labelledby="nav-food-tab"
                ...
                <div class="collapse" id="collapseExample">
                    This is the foodpicker. Here you can find the foods that are relevant to you. You can go to the foodview
                    by clicking a food of your chosing from this picker.
                    You can select more items per page to be shown via the 'per page' picker.
                    The homepage shows 24 foods when first opening the page. You can also go to the different pages via the
                    pagination buttons at the bottom.
                    Do note that while searching, this pagination does not get reset. This means that f.e. when you are
                    filtering for the word 'pineapple' and you are not on the first page, the search result will show 'no
                    results found'.
                    This is because you are searching on the page that you have selected while there is only one pineapple
                    food which will be shown on the first page. You can find the pineapple food, or any other food for that
                    matter, by going to the first page.
                    These foods are taken from the <a href="/about">FooDB dataset</a>.
                </div>
            ...
            <!-- Nutrients -->
            <div class="tab-pane fade rows" id="nav-nutrition" role="tabpanel" aria-labelledby="nav-nutrition-tab"
                tabindex="0">
                <div class="collapse" id="collapseExample">
                    This is the Nutrient/Nutrition Table. Here you can get a complete overview of the currently filtered
                    foods. The unit are all shown in 100g/mg.
                    You can sort by a nutrient by clicking on the table header column. The arrow will indicate if you are
                    ascending/descending the results. You can also go to the foodview of a food by clicking name of the food
                    in the name column.
                    Do note that the value NaN stands for 'No Available Number'. This means that there are no measurements
                    available for this food.
                ...
                <table v-if="tabIndex == 1" class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <TableRowNutrition v-bind:key="foodIDs[item.name]" v-if="Object.keys(sortedFoodNames).length != 0"
                            v-for="item in sortedFoodNames" :columnNames="nutritionHeaders" :id="foodIDs[item.name]"
                            :name="item.name" :items="foodNutritions[item.name] ?? []" :max_value="getMaxColums()" />
                    </tbody>
                </table>
            </div>
            <!-- Allergies -->
            <div class="tab-pane fade rows" id="nav-allergies" role="tabpanel" aria-labelledby="nav-allergies-tab"
                tabindex="0">
                <div class="collapse" id="collapseExample">
                    This is the Allergy Table. Here you can get an overview of the allergies in the currently filtered food
                    selection.
                    You can hover over a bar at the top to get a 'The amount the allergy is found on the page' / 'The amount
                    of foods shown on this page' ratio.
                    You can again sort the table via clicking the table header columns (See Nutrient/Nutrition Table for
                    more info).
                    These allergies are taken from the <a href="/about">Allergy dataset</a>.
                </div>
                <table v-if="tabIndex == 2" class="table table-hover">
                    <thead>
                        <TableGraph :percentages="getAllergyPercentages" :columnNames="getAllergyNames"
                            :counts="getAllergyCount" :totaalFoods="foodItems.length" />
                    </thead>
                    <thead class="table-secondary">
                    ...
                    <tbody>
                        ...
                        <TableRowAllergy v-if="foodItems.length != 0" v-for="item in sortedFoodNames"
                            :id="foodIDs[item.name]" :name="item.name" :allergyNames="getAllergyNames"
                            :allergies="getAllergiesOfFood(item.name)" />
                    </tbody>
                </table>
            </div>
            <div class="tab-pane fade no-rows" id="nav-recipes" role="tabpanel" aria-labelledby="nav-recipes-tab"
                tabindex="0">
                <div class="collapse" id="collapseExample">
                        This is the Recipe Count List. Here we can see the amount of recipes found for all the currently
                        filtered foods.
                ...
                <div v-if="!isLoading || recipesCount.length != 0">
                    <TableRecipe v-if="tabIndex == 3" class="mx-auto" :foodIds="foodIds" :label="recipeLabel"
                        :recipesCount="recipesCount"></TableRecipe>
                </div>
            </div>
        </div>
    </div>
...
<style scoped lang="scss">@import "@/../scss/custom.scss";
